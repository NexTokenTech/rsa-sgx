...
use super::dbig::DBig;
use crate::{
	arch::{self, Chunk, DChunk},
	rand::RAND,
};

use serde::*;
use std::{cmp::Ordering, format, string::String};

pub use super::rom::{BASEBITS, MODBYTES};
...
impl Big {
	...
	pub fn from_byte_array(b: &[u8], n: usize) -> Big {
		...
		let len = if b.len() >= max_big { max_big as usize } else { b.len() };
